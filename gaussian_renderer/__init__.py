...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        debug=False,
        # pipe.debug
        freeze_low_beta=getattr(pipe, "freeze_low_beta", False),
    )
    ...
    # --- Alpha safety (paper-style) ---
    render_alpha = torch.clamp(render_alpha, min=1e-6)
    ...
    render_normal = render_normal / torch.clamp(norm, min=1e-6)
    ...
    render_depth_expected = render_depth_expected / torch.clamp(render_alpha, min=1e-6)
