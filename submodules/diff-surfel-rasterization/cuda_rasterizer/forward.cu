	...
	float* __restrict__ out_others)
{
	...
	// --------------------------------------------------
	// gsplat-style pixel validity guard (CUDA-safe)
	// --------------------------------------------------
	bool pixel_invalid = (alpha_accum < 1e-4f);

	// Write output for invalid pixels WITHOUT returning
	if (pixel_invalid) {
		if (inside) {
			final_T[pix_id] = 1.0f;
			n_contrib[pix_id] = 0;
...
#if RENDER_AXUTILITY
			out_others[pix_id + ALPHA_OFFSET * H * W] = 0.0f;
			out_others[pix_id + DEPTH_OFFSET * H * W] = 0.0f;
			out_others[pix_id + DISTORTION_OFFSET * H * W] = 0.0f;
			for (int ch = 0; ch < 3; ch++)
				out_others[pix_id + (NORMAL_OFFSET + ch) * H * W] = 0.0f;
#endif
    		}
	}
...
if (inside && !pixel_invalid)
{
    ...
    // SAFE NORMAL NORMALIZATION (correcto, déjalo igual)
