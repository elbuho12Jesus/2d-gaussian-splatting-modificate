	...
	float* __restrict__ dL_dcolors)
{
	...
	const bool pixel_invalid = inside && (1.0f - T_final < 1e-4f);
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (rho3d <= rho2d) {
				...
				// dρ/ddelx = 2 * FilterInvSquare * d.x
				// dρ/ddely = 2 * FilterInvSquare * d.y
				float d_rho_ddelx = 2.0f * FilterInvSquare * d.x;
				float d_rho_ddely = 2.0f * FilterInvSquare * d.y;

				atomicAdd(&dL_dmean2D[global_id].x, dL_d_rho * d_rho_ddelx);
				atomicAdd(&dL_dmean2D[global_id].y, dL_d_rho * d_rho_ddely);
