...
def post_process_mesh(mesh, cluster_to_keep=1000):
    ...
    if len(mesh.vertices) == 0 or len(mesh.triangles) == 0:
        print("[WARN] Mesh is empty, skipping post-processing.")
        return mesh
    ...
    # ✅ GUARD: no hay clusters
    if cluster_n_triangles.size == 0:
        print("[WARN] No triangle clusters found, skipping post-processing.")
        return mesh
...
class GaussianExtractor(object):
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            ...
            # new depth
            depth = render_pkg['render'][0:1]  # render_depth_expected
    ...
    def extract_mesh_unbounded(self, resolution=1024):
        ...
        def compute_sdf_perframe(i, points, depthmap, rgbmap, viewpoint_cam):
            ...
            """
            #old new_points and pix_coords
            '''
            new_points = torch.cat([points, torch.ones_like(points[...,:1])], dim=-1) @ viewpoint_cam.full_proj_transform
            z = new_points[..., -1:]
            pix_coords = (new_points[..., :2] / new_points[..., -1:])
            '''
            #new new_points and pix_coords
            points_h = torch.cat([points, torch.ones_like(points[..., :1])], dim=-1)
            cam_points = points_h @ viewpoint_cam.world_view_transform.T
            z = cam_points[..., 2:3]   # ✅ depth real en cámara
            # proyectar a NDC
            proj_points = cam_points @ viewpoint_cam.projection_matrix.T
            pix_coords = proj_points[..., :2] / proj_points[..., 2:3]

            #old mask_proj
            #mask_proj = ((pix_coords > -1. ) & (pix_coords < 1.) & (z > 0)).all(dim=-1)
            #new mask_proj
            mask_proj = ((pix_coords > -1.) & (pix_coords < 1.)).all(dim=-1)
        ...
        def compute_unbounded_tsdf(samples, inv_contraction, voxel_size, return_rgb=False):
            ...
            if inv_contraction is not None:
                ...
                base_trunc = max(20 * voxel_size, 0.05)
                sdf_trunc = base_trunc * torch.ones_like(samples[:, 0])
                sdf_trunc[mask] *= 1/(2-torch.linalg.norm(samples, dim=-1)[mask].clamp(max=1.9))
                samples = inv_contraction(samples)
            else:
                sdf_trunc = max(20 * voxel_size, 0.05)
            ...
            for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="TSDF integration progress"):
                ...
                if tsdfs.numel() == 0:
                    return tsdfs if not return_rgb else (tsdfs, rgbs)
                if tsdfs.numel() > 0:
                    print("SDF stats:", tsdfs.min().item(), tsdfs.max().item())
